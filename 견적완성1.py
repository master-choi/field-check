...
UNIT_PRICES = {
    ...
    # 창호 (1개당, 기본값)
    'window_base': {'material': 350000, 'labor': 70000, 'expense': 1400},
    # 창호 부대공정 (m당)
    ...
    'window_removal': {'material': 0, 'labor': 3377, 'expense': 0},
    # 공통경비
...
with col_w:
    length_ns = st.number_input("가로 (남/북벽 길이)", value=2900, step=100)   # 사용자 정의: 가로 = 남/북
with col_l:
    width_ew = st.number_input("세로 (동/서벽 길이)", value=2500, step=100)     # 세로 = 동/서
with col_h:
...
wall_ns_area = len_ns_m * h_m      # 남/북 벽 각각 면적
wall_ew_area = wid_ew_m * h_m      # 동/서 벽 각각 면적
...
for i, (win_w, win_h) in enumerate(windows):
    ...
    if key in WINDOW_PRICES:
        wp = WINDOW_PRICES[key]
    else:
        wp = p['window_base']
    total_material += wp['material']
    total_labor += wp['labor']
    total_expense += wp['expense']
    details.append(f"창문 {i+1} ({win_w}x{win_h}mm) 설치: 자재 {wp['material']:,.0f} + 노무 {wp['labor']:,.0f} + 경비 {wp['expense']:,.0f}")
    ...
    # 창짝 철거 (4개 가정)
    lab_rem = p['window_removal']['labor'] * 4
    total_labor += lab_rem
    details.append(f"  - 창짝 철거(4개): 노무 {lab_rem:,.0f}")
